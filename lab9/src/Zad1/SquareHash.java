...
public class SquareHash {
    ...
    private int H(Integer key){
        ...
        private int pSquare(int i ){
            return 2*i+4*i*i;
        }

        public boolean put(int key,String value){
            int index = (H(key)+ pSquare(0))%arr.length;
            int i=0;
            while (arr[index]!=null&& i<=arr.length){
                index = (H(key)+ pSquare(i))%arr.length;
                i++;
            }
            if(i==arr.length){
                return false;
    ...
    public void wypiszStatystyki(){
        System.out.println("Dla 100 wyszukan w tablicy o rozmiarze 1000 kwadratowe ");
